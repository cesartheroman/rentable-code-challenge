...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        integration_api_url = (
            "http://localhost:8009/api/simulated-pms-integration-api/transactions/"
        )
        try:
        ...
        for transaction_data in transactions_data:
            ...
            try:
                ...
                self.stderr.write(
                    f'Tenant with ID {tenant_id} not found. Skipping transaction {transaction_data.get("id")}.'
                )
                continue

            # Create or update Transaction object
            # DONE: The 'type' field is intentionally omitted here for the candidate to add.
            Transaction.objects.update_or_create(  # type: ignore
                id=transaction_data.get(
                    "id"
                ),  # Assuming 'id' is unique for transactions
                defaults={
                    "tenant": tenant,
                    "date": datetime.strptime(
                        transaction_data.get("date"), "%Y-%m-%d"
                    ).date(),
